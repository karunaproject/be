...
class InMemoryVisitorMessageRepository implements VisitorMessageRepository{
    ...
    private static final HashMap<Long, VisitorMessage> database = new HashMap<>();
    ...
    public <S extends VisitorMessage> S save(S entity) {
        long id = database.keySet().size() + 1;
        VisitorMessage savedEntity = new VisitorMessage(id, OffsetDateTime.now(), entity.getBody(), entity.getContact());
